...
const { tcping, ping, checkCertificate, checkStatusCode, getTotalClientInRoom, setting, mssqlQuery, postgresQuery, mysqlQuery, oracledbQuery, setSetting, httpNtlm, radius, grpcQuery,
    ...
    toJSON(preloadData = {}, includeSensitiveData = true) {
        ...
        if (includeSensitiveData) {
            data = {
                ...
                oracleDatabaseUser: this.oracleDatabaseUser,
                oracleDatabasePassword: this.oracleDatabasePassword,
            };
        }
        ...
        const beat = async () => {
            ...
            try {
                if (await Monitor.isUnderMaintenance(this.id)) {
                    ...
                    let startTime = dayjs().valueOf();
                    // Provide a default query if none is set
                    const query = this.databaseQuery ? this.databaseQuery : "SELECT 1 FROM DUAL";
                    log.debug("monitor", "Before calling oracledbQuery with config: " + JSON.stringify({
                        user: this.oracleDatabaseUser,
                        password: this.oracleDatabasePassword ? "********" : null,
                        connectString: this.databaseConnectionString,
                        databaseQuery: query
                    }));
                
                    await oracledbQuery({
                        user: this.oracleDatabaseUser,
                        password: this.oracleDatabasePassword,
                        connectString: this.databaseConnectionString
                    }, query);
