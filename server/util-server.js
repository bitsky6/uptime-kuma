...
/**
 * Run a query on Oracle DB using JDBC connection string.
 * @param {object} config An object containing user, password, and connectString.
 * @param {string} query The query to execute.
 * @returns {Promise<void>}
 */
exports.oracledbQuery = async function (config, query) {
    try {
        log.debug("oracledb", "Received config:", JSON.stringify(config));

        if (!config.connectString) {
            throw new Error("Missing connection string in config");
        }
        let cleanConnectString = config.connectString.trim();

        // Regex for new (Easy Connect) format:
        // Matches strings that start with "jdbc:oracle:thin:@//"
        const newRegex = /^jdbc:oracle:thin:@\/\/(.+)$/i;
        // Regex for old style format:
        // Matches strings that start with "jdbc:oracle:thin:@" followed by hostname:port:sid (without the double slash)
        const oldRegex = /^jdbc:oracle:thin:@([^\/]+)$/i;

        let match = cleanConnectString.match(newRegex);
        if (match) {
            cleanConnectString = match[1];
        } else {
            match = cleanConnectString.match(oldRegex);
            if (match) {
                cleanConnectString = match[1];
                // For the old format, the connection string will be in "hostname:port:sid" format.
                // Oracle expects this format for older versions.
            } else {
                throw new Error("Connection string does not match expected Oracle JDBC formats");
            }
        }

        // Depending on your driver’s requirements, you might need to ensure that
        // the clean connection string begins with '//' (for the Easy Connect format).
        // For an old-style connection string (hostname:port:sid), you might leave it as-is.
        // In this example, we'll add '//' only if the string already contains a '/' character.
        if (cleanConnectString.includes('/')) {
            if (!cleanConnectString.startsWith('//') && !cleanConnectString.startsWith('(')) {
                cleanConnectString = '//' + cleanConnectString;
            }
        }
        config.connectString = cleanConnectString;

        let connection = await oracledb.getConnection(config);
        await connection.execute(query);
        await connection.close();
    } catch (error) {
        console.error("OracleDB query error:", error);
        throw error;
    }
};
