                ...
                <div class="shadow-box shadow-box-with-fixed-bottom-bar">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <!-- SQL Server / PostgreSQL / MySQL / Redis / MongoDB / OracleDB -->
                            ...
                            <template v-if="monitor.type === 'mysql'">
                                <div class="my-3">
                                    ...
                                    <!-- TODO: Rename monitor.radiusPassword to monitor.password for general use -->
                                    <HiddenInput id="mysql-password" v-model="monitor.radiusPassword" autocomplete="false"> required="true"</HiddenInput>
                                </div>
                            </template>

                            <template v-if="monitor.type === 'oracledb'">
                                <div class="my-3">
                                    <h3 class="mb-2">OracleDB Configuration</h3>

                                    <div class="my-3">
                                        <label for="oracleDatabaseUser" class="form-label">{{ $t("User") }}</label>
                                        <input
                                            id="oracleDatabaseUser"
                                            v-model="monitor.oracleDatabaseUser"
                                            type="text"
                                            class="form-control"
                                            required
                                            placeholder="e.g., system"
                                        >
                                        <label class="form-label">Enter the database user name.</label>
                                    </div>

                                    <div class="my-3">
                                        <label for="oracleDatabasePassword" class="form-label">{{ $t("Password") }}</label>
                                        <HiddenInput id="oracleDatabasePassword" v-model="monitor.oracleDatabasePassword" autocomplete="false"></HiddenInput>
                                        <label class="form-label">Enter the password for the database user.</label>
                                    </div>

                                    <div class="my-3">
                                        <label for="databaseConnectionString" class="form-label">Connection String</label>
                                        <input
                                            ...
                                            class="form-control"
                                            required
                                            placeholder="jdbc:oracle:thin:@//hostname:port/service_name or hostname:port/service_name"
                                        >
                                        <label class="form-label">
                                            Specify the connection string for your Oracle database.<br>
                                            You can use a JDBC connection string (e.g., <code>jdbc:oracle:thin:@//localhost:1521/ORCLPDB</code>) or a plain Easy Connect string (e.g., <code>localhost:1521/ORCLPDB</code>).<br>
                                            You can also use to connect to older Oracle versions with <code>jdbc:oracle:thin:@localhost:port:sid</code>)
                                            If a JDBC string is entered, the prefix will be automatically removed.
                                        </label>
                                    </div>

                                    <div class="my-3">
                                        <label for="databaseQuery" class="form-label">Query</label>
                                        <textarea
                                            id="databaseQuery"
                                        ...
                                        ></textarea>
                                        <label class="form-label">
                                            Enter the query to execute on your Oracle database. If left empty, it will default to <code>SELECT 1 FROM DUAL</code>.
                                        </label>
                                    </div>
                                </div>
                            </template>
    ...
    data() {
        return {
            ...
            connectionStringTemplates: {
                ...
                "oracledb": "jdbc:oracle:thin:@//hostname:port/service_name or hostname:port/service_name",
            },
            draftGroupName: null,
